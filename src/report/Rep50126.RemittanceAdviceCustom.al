// namespace Obrien.Purchases.Reports;
...
report 50126 "Remittance Advice - Custom"
{
    DefaultLayout = RDLC;
    WordLayout = './Src/Layout/RemittanceAdviceEntries.docx';
    Caption = 'Remittance Advice - Custom';
    ...
    dataset
    {
        dataitem("Vendor Ledger Entry"; "Vendor Ledger Entry")
        {
            ...
            column(CompanyAddr1; UpperCase(CompanyAddr[1]))
            {
            }
            column(VendorAddr1; VendorAddr[1])
            {
            }
            column(CompanyAddr2; UpperCase(CompanyAddr[2]))
            {
            }
            column(VendorAddr2; VendorAddr[2])
            {
            }
            column(CompanyAddr3; UpperCase(CompanyAddr[3]))
            {
            }
            column(VendorAddr3; VendorAddr[3])
            {
            }
            column(CompanyAddr4; UpperCase(CompanyAddr[4]))
            {
            }
            column(VendorAddr4; VendorAddr[4])
            {
            }
            column(CompanyAddr5; UpperCase(CompanyAddr[5]))
            {
            }
            column(External_Document_No_; "External Document No.")
            {
            }
            column(CompanyAddr6; UpperCase(CompanyAddr[6]))
            {
            }
            ...
            column(CompanyInfoEmail; CompanyInfo."AP E-Mail")
            {
            ...
            dataitem(VendLedgEntry2; "Vendor Ledger Entry")
            {
                ...
                column(NegAmountLCY_VendLedgEntry2; -"Amount (LCY)")
                {
                    ...
                    begin
                        ...
                        if VendorList.RunModal() = Action::LookupOK then begin
                            ...
                            if Vendor.FindSet() then
                                repeat
                                    count := Vendor.count();
                                    if OldCusNo <> Vendor."No." then
                                        Counter := Counter + 1;
                                    if Count > Counter then begin
                                        VendorNo += format(Vendor."No.") + '|'
                                    end else
                                        VendorNo += format(Vendor."No.");
                                    OldCusNo := Vendor."No.";
                                until Vendor.Next() = 0;
    ...
    var
        ...
        AmtCaptionLbl: Label 'Payment Amount';
        PmtDiscTakenCaptionLbl: Label 'Discount';
        RemAmtCaptionLbl: Label 'Remaining Amount';
        OriginalAmtCaptionLbl: Label 'Original Amount';
        YourDocNoCaptionLbl: Label 'Document No.';
        OurDocNoCaptionLbl: Label 'Document No.';
        CurrCodeCaptionLbl: Label 'Currency';
