report 50115 "Customer Statement Cust"
{
    Caption = 'Customer Statement Custom';
    ApplicationArea = All;
    UsageCategory = ReportsAndAnalysis;
    // DefaultRenderingLayout = "CustomStatement.docx";
    // WordMergeDataItem = Customer;
    DefaultLayout = RDLC;
    RDLCLayout = './src/Layout/CustomerStatementpk.rdl';
    ...
    dataset
    {
        dataitem(Customer; Customer)
        {
            ...
            column(No_Cust; "No.") { }
            column(CompanyInfo_Add; CompanyInfoPK.Address) { }
            column(CompanyInfo_Add2; CompanyInfoPK."Address 2") { }
            column(CompanyInfo_City; CompanyInfoPK.City) { }
            column(CompanyInfoCity; CompanyInfoPK.County) { }
            column(CompanyInfoPotCode; CompanyInfoPK."Post Code") { }
            column(CompanyInfoPH1; CompanyInfoPK."Phone No.") { }
            column(CompanyInfoph2; CompanyInfoPK."Phone No. 2") { }
            // DCS:: HP 22092025 ++
            column(CompanyInfo_EMail; CompanyInfoPK."AP E-Mail") { }
            // DCS:: HP 22092025 --
            column(CompanyInfo_HomePage; CompanyInfoPK."Home Page") { }
            column(CompanyInfoNAme; CompanyInfoPK.Name) { }
            column(CompanyInfoABN; CompanyInfoPK.ABN) { }
            column(CompanyInfoACN; CompanyInfoPK."IRD No.") { }
            ...
            dataitem("Integer"; "Integer")
            {
                ...
                dataitem(CurrencyLoop; "Integer")
                {
                    ...
                    dataitem(CustLedgEntryHdr; "Integer")
                    {
                        ...
                        {
                            AutoFormatExpression = CustomCurrency2.Code;
                            AutoFormatType = 1;
                        }
                        ...
                        dataitem(DtldCustLedgEntries; "Detailed Cust. Ledg. Entry")
                        {
                            ...
                            column(DocNo_DtldCustLedgEntries; Format("Document No." + ' / ' + PONo))
                            {
                            }
                            ...
                            column(DueDate_DtldCustLedgEntries; duedays)
                            {
                            }
                            ...
                            column(Currency2Code; CustomCurrency2.Code)
                            {
                            }
                            ...
                            var
                                ...
                                projectLedEnt: Record "Job Ledger Entry";
                            begin
                                Clear(DueDays);
                                ...
                                case "Entry Type" of
                                    "Entry Type"::"Initial Entry":
                                        begin
                                            ...
                                            // CustLedgerEntry.SetFilter("Remaining Amount", '>0'); //YK
                                            ...
                                            if SalesInvHdr.Get(CustLedgerEntry."Document No.") then begin
                                                ...
                                                PONo := SalesInvHdr."Customer PO Number";
                                                // Message(PONo);
                                            end;
                                            projectLedEnt.reset();
                                            projectLedEnt.SetRange("Entry Type", projectLedEnt."Entry Type"::Sale);
                                            projectLedEnt.SetRange("Document No.", CustLedgerEntry."Document No.");
                                            if projectLedEnt.FindFirst() then
                                                PONo += ' / ' + projectLedEnt."Job No.";
                                            //PK 22Sept2025-
                                ...
                                //     DueDays := EndDate - DueDate;
                                // DCS::HP13112025 ++
                                if (DueDate <> 0D) and (DueDate <= EndDate) then
                                    DueDays := EndDate - DueDate
                                else
                                    DueDays := 0;
                                // DCS::HP13112025 ++
                    ...
                    dataitem(OverdueVisible; "Integer")
                    {
                        ...
                        dataitem(CustLedgEntry2; "Cust. Ledger Entry")
                        {
                            ...
                            column(DueDate_CustLedgEntry2; duedays)
                            {
                            }
                            ...
                            column(Currency2Code_CustLedgEntry2; CustomCurrency2.Code)
                            {
                            }
                            ...
                            begin
                                ...
                                // SetFilter("Remaining Amount", '>0'); //YK
                                SetRange("Currency Code", CustomCurrency2.Code);
                    ...
                    begin
                        if Number = 1 then
                            // CustomCurrency2.SetFilter(CustomCurrency2.Code, '<>%1', ''); //PK 22Sep2025
                            CustomCurrency2.Find('-');
                        ...
                        repeat
                            ...
                            if not IsFirstLoop then
                                ...
                                if CustomCurrency2.Next() = 0 then
                                    CurrReport.Break();
                            ...
                            CustLedgerEntry.SetRange("Currency Code", CustomCurrency2.Code);
                        ...
                        if StatementStyle = StatementStyle::"Open Item" then begin
                            ...
                            CustLedgerEntryBalance.SetRange("Currency Code", CustomCurrency2.Code);
                            ...
                            OnCurrencyLoopOnAfterGetRecordOnAfterCalculateStartBalanceStatementTypeOpenItems(Customer, CustLedgerEntryBalance, StartDate, EndDate, CustomCurrency2.Code, StartBalance);
                            CustBalance := StartBalance;
                        end
                        else begin
                            Cust2 := Customer;
                            Cust2.SetRange("Date Filter", 0D, StartDate - 1);
                            Cust2.SetRange("Currency Filter", CustomCurrency2.Code);
                            Cust2.CalcFields("Net Change");
                            StartBalance := Cust2."Net Change";
                            OnCurrencyLoopOnAfterGetRecordOnAfterCalculateStartBalanceStatementTypeBalance(Customer, StartDate, EndDate, CustomCurrency2.Code, StartBalance);
                    ...
                    trigger OnPreDataItem()
                    begin
                        Customer.CopyFilter("Currency Filter", CustomCurrency2.Code);
                    end;
                }
                dataitem(AgingBandVisible; "Integer")
                {
                    ...
                    dataitem(AgingBandLoop; "Integer")
                    {
                        ...
                        column(AgingDate1; agingdateheader4)
                        {
                        }
                        ...
                        column(AgingDate21; agingdateheader3)
                        {
                        }
                        ...
                        column(AgingDate31; agingdateheader2)
                        {
                        }
                        ...
                        column(AgingDate41; agingdateheader1)
                        {
                        }
                        ...
                        column(beforeCaption; agingdateheader5)
                        {
                        }
            ...
            begin
                CompanyInfoPK.Get();
                CompanyInfo.Get();
                CurrReport.SetTableView(Customer);
                ...
                RunEmailForSelectedCustomers(CustomerFilter);
            ...
            area(content)
            {
                ...
                group(Customer)
                {
                    field(CustomerFilter; CustomerFilter)
                    {
                        ...
                        ToolTip = 'Select multiple customers';
                        trigger OnLookup(var Text: Text): Boolean
                        var
                            Cust: Record Customer;
                            PipeSeparated: Text;
                        begin
                            // Let user multi-select customers from list
                            if PAGE.RunModal(PAGE::"Customer List", Cust) = ACTION::LookupOK then begin
                                // Multi-selected records are stored in filter (via selection filter)

                                if Cust.FindSet() then
                                    repeat
                                        if PipeSeparated <> '' then
                                            PipeSeparated += '|';
                                        PipeSeparated += Cust."No.";
                                    until Cust.Next() = 0;

                                CustomerFilter := PipeSeparated;
                                Text := PipeSeparated;
                                exit(true);
                            end;
                    ...
                    //             customerlist.SetRecord(customer2);
                    ...
                    //                         CustomerFilter += format(customer2."No.") + '|'
                    //                     end else
                    //                         CustomerFilter += format(customer2."No.");
    ...
    //     // // }
    //     // layout("CustomStatement.docx")
    //     // {
    //     //     Type = Word;
    //     //     LayoutFile = './src/Layout/CustomStatement.docx';
    //     //     Caption = 'Custom Customer Statement (Word)';
    //     //     Summary = 'The Standard Customer Statement (Word) provides a basic layout.';
    //     // }
    //     // layout("CustomerStatementEmailBody.docx")
    //     // {
    //     //     Type = Word;
    //     //     LayoutFile = './src/Layout/CustomerStatementEmailBody.docx';
    //     //     Caption = 'Customer Statement Email Body';
    //     //     Summary = 'The Standard Customer Statement (Word) provides a basic layout.';
    //     // }
    //     layout("CustomerStatement RDLC")
    //     {
    //         Type = RDLC;
    //         LayoutFile = './src/Layout/CustomerStatementpk.rdl';
    ...
    var
        ...
        CustomCurrency2: Record Currency temporary;
        CompanyInfoPK: Record "Company Information";

        //CustomCurrency2: Record CustomCurrency;
        ...
        DueDate_CustLedgEntry2CaptionLbl: Label 'No. of Due days';
    ...
    local procedure SplitPipeSeparated(InputText: Text): List of [Text]
    var
        OutList: List of [Text];
        Token: Text;
    begin
        while StrLen(InputText) > 0 do begin
            if StrPos(InputText, '|') > 0 then begin
                Token := CopyStr(InputText, 1, StrPos(InputText, '|') - 1);
                InputText := CopyStr(InputText, StrPos(InputText, '|') + 1);
            end else begin
                Token := InputText;
                InputText := '';
            end;
            OutList.Add(DelChr(Token, '=', ' ')); // Clean whitespace
        end;
        exit(OutList);
    end;
    ...
    begin
        ...
        AgingDate[3] := AgingBandEndingDate;
        AgingDate[2] := CalcDate(PeriodLength2, AgingDate[3]);
        AgingDate[1] := CalcDate(PeriodLength2, AgingDate[2]);
        ...
        // AgingDateHeader1 := Format(AgingDate[4] - AgingDate[4] + 1) + ' - ' + FORMAT(EndDate - AgingDate[4]) + ' days';
        //AgingDateHeader2 := Format(AgingDate[4] - AgingDate[3] + 1) + ' - ' + Format(EndDate - AgingDate[3]) + ' days';
        AgingDateHeader1 := Format(AgingDate[2] - AgingDate[2] + 1) + ' - ' + Format(EndDate - AgingDate[2]) + ' days';
        AgingDateHeader2 := Format(AgingDate[2] - AgingDate[1] + 1) + ' - ' + Format(EndDate - AgingDate[1]) + ' days';
        AgingDateHeader3 := 'After ' + Format(EndDate - AgingDate[1]) + ' days';
    end;
    ...
    begin
        ...
        I := 1;
        GoOn := true;
        while (I <= 3) and GoOn do begin//PK 5
    ...
    procedure RunEmailForSelectedCustomers(CustomerFilter: Text)
    var
        Customers: List of [Text];
        CustomerNo: Text;
        Customer: Record Customer;
        EmailCodeunit: Codeunit "Email Attachment Customer"; // Your custom email handler
        i: Integer;
    begin
        if CustomerFilter = '' then
            exit;

        // Split CustomerFilter string like "C00010|C00020"
        Customers := SplitPipeSeparated(CustomerFilter);

        for i := 1 to Customers.Count() do begin
            CustomerNo := Customers.Get(i);
            if Customer.Get(CustomerNo) and (Customer."E-Mail" <> '') then begin
                // EmailCodeunit.EmailCustomerStatement(CustomerNo); // You’ll create this
            end;
        end;
    end;
    ...
    begin
        ...
        EndDate := NewEndDate;
        CustomerFilter := CutstomCutomerFilter;
    end;
    ...
    begin
        CustomCurrency2.Reset();
        CustomCurrency2.DeleteAll();
        CustLedgerEntry.Reset();
        CustLedgerEntry.SetCurrentKey("Currency Code");
        CustomCurrency2.Init();
        while CustLedgerEntry.FindFirst() do begin
            CustomCurrency2.Code := CustLedgerEntry."Currency Code";
            CustomCurrency2.Insert();
